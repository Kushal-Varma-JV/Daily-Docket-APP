...
from utils.constants import (
    ...
    DEFAULT_OA_BASE, DEFAULT_WORKFLOW_ID,
)
...
from tabs.esso_token_tab import display_esso_token_tab
from tabs.ai_chat_tab import display_ai_chat_tab
...
# ══════════════════════════════════════════════════════════════════════════
#  STANDARD 11-FIELD SCHEMA  (single source of truth for the whole app)
# ══════════════════════════════════════════════════════════════════════════
STANDARD_COLUMNS = [
    "Date",
    "Case Number",
...
def _normalize_to_standard(cases: list, court_name_fallback: str) -> list:
    ...
    into the standard 11-field schema.

    Handles these formats:

      Snake_case (Circuit 1 reference style):
          date, case_number, case_name, nature_of_case, court_name,
          location, judges_panel, courtroom, purpose_of_hearing, time,
          description

      Title Case 10-field (some circuits):
      ...
      11-field style (Circuits 9, 10):
          Case Name, Case Number, Nature of Case, Court Name,
          Location, Judges / Panel, Courtroom,
          Purpose of Hearing, Date, Time, Description

      Circuit 4 style (combined court/location):
    ...
    for c in cases:
        ...
        # ── Detect format ──
        is_snake     = "case_name" in row and "court_name" in row
        ...
        if is_snake:
            # Circuit 1 reference format
            normalized = {
                "Date":               row.get("date", ""),
                "Case Number":        row.get("case_number", ""),
                "Case Name":          row.get("case_name", ""),
                "Nature of Case":     row.get("nature_of_case", ""),
                "Court Name":         row.get("court_name", court_name_fallback),
                "Location":           row.get("location", ""),
                "Judges/Panel":       row.get("judges_panel", ""),
                "Courtroom":          row.get("courtroom", ""),
                "Purpose of Hearing": row.get("purpose_of_hearing", "Oral Argument"),
                "Time":               row.get("time", ""),
...
# ══════════════════════════════════════════════════════════════════════════
#  NOTE: update_combined_cases() has been REMOVED.
#  The combined tab now reads directly from each circuit's session-state
#  key at render time via _collect_all_data(), so data is always fresh.
# ══════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    with st.sidebar:
            ...
            if st.button("📥 Fetch First Circuit", type="primary", key="fetch_c1"):
                with st.spinner("Fetching First Circuit PDF..."):
                    try:
                        ...
                        st.success(
                            f"✅ First Circuit: "
                            f"{len(st.session_state.c1_parsed_cases)} cases"
                        )
        ...
        if circuit_option in ["Second Circuit", "All Circuits"]:
            ...
            with col1:
                c2_start_month = st.selectbox(
                    "Start Month", range(1, 13), index=0, key="c2_sm"
                )
                c2_start_year = st.number_input(
                    "Start Year", 2020, 2030, 2026, key="c2_sy"
                )
            with col2:
                c2_end_month = st.selectbox(
                    "End Month", range(1, 13), index=2, key="c2_em"
                )
                c2_end_year = st.number_input(
                    "End Year", 2020, 2030, 2026, key="c2_ey"
                )
            ...
            if st.button("📥 Fetch Fourth Circuit", type="primary", key="fetch_c4"):
                ...
                try:
                    ...
                    def c4_progress(current, total, label):
                        status_placeholder.info(
                            f"📄 Parsing PDF {current}/{total}: {label}"
                        )
                        if total > 0:
                ...
                try:
                    ...
                    def c5_progress(stage, label, current, total):
                        if stage == "months":
                            ...
                            status_placeholder.info(
                                f"📅 Month {current}/{total}: {label}"
                            )
                            if total > 0:
                                progress_bar.progress(current / total * 0.3)
                        elif stage == "cases":
                            status_placeholder.info(
                                f"⚖️ Hearing {current}/{total}: {label}"
                            )
                            if total > 0:
                                progress_bar.progress(
                                    0.3 + current / total * 0.7
                                )
        ...
        if circuit_option in ["Eighth Circuit", "All Circuits"]:
            ...
            if st.button("📥 Fetch Eighth Circuit", type="primary", key="fetch_c8"):
                ...
                progress_bar = st.progress(0)
                try:
                    scraper = USCA8Scraper(
                        index_url=c8_url, verify_ssl=False
                    )

                    def c8_progress(current, total, label):
                        status_placeholder.info(
                            f"📄 Downloading & parsing PDF "
            ...
            if st.button("📥 Fetch Tenth Circuit", type="primary", key="fetch_c10"):
                ...
                try:
                    ...
                    def c10_progress(current, total, label):
                        status_placeholder.info(
                            f"📄 Processing {current}/{total}: {label}"
                        )
                        if total > 0:
                            progress_bar.progress(current / total)
        ...
        if circuit_option in ["Eleventh Circuit", "All Circuits"]:
            ...
            st.caption(
                "Parses calendars posted 4 weeks before oral argument sessions."
            )
            if st.button(
                "📥 Fetch Eleventh Circuit", type="primary", key="fetch_c11"
            ):
                status_placeholder = st.empty()
                progress_bar = st.progress(0)
                try:
        ...
        if circuit_option in ["DC Circuit", "All Circuits"]:
            ...
            st.caption(
                "Scrapes future oral argument calendar with live audio streaming."
            )
        ...
        if circuit_option in ["Federal Circuit", "All Circuits"]:
            ...
            if st.button(
                "📥 Fetch Federal Circuit", type="primary", key="fetch_cafc"
            ):
                status_placeholder = st.empty()
                progress_bar = st.progress(0)
                try:
                    ...
                    def cafc_progress(current, total, label):
                        status_placeholder.info(
                            f"📄 Parsing {current}/{total}: {label}"
                        )
                        if total > 0:
                            progress_bar.progress(current / total)

                    all_cafc = scraper.scrape_all(
                        progress_callback=cafc_progress
                    )
                    ...
                    status_placeholder.empty()
                    st.success(
                        f"✅ Federal Circuit: {len(all_cafc)} cases"
                    )
                except Exception as e:
                    st.error(f"❌ Error: {str(e)}")
            st.divider()
        ...
        # ── Open Arena Settings ──
        st.subheader("🤖 Open Arena Settings")
        st.session_state.oa_base = st.text_input(
            "Base URL", value=st.session_state.oa_base
        )
        st.session_state.workflow_id = st.text_input(
            "Workflow ID", value=st.session_state.workflow_id
        )
        st.warning("⚠️ SSL verification is disabled")
