"""Tenth Circuit tab display — standardized 11-field schema."""
import json
import streamlit as st
import pandas as pd
from datetime import datetime
from utils.helpers import safe_sorted_unique


def display_tenth_circuit_tab():
    st.title("🔵 Tenth Circuit Court Calendar")

    if st.session_state.c10_cases:
        df = pd.DataFrame(st.session_state.c10_cases)

        # --- Metrics row ---
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Total Cases", len(df))
        with col2:
            if "Date" in df.columns:
                st.metric("Hearing Days", df["Date"].dropna().nunique())
            else:
                st.metric("Hearing Days", "N/A")
        with col3:
            if "Judges / Panel" in df.columns:
                panels = df["Judges / Panel"].dropna()
                panels = panels[panels != ""]
                st.metric("Judge Panels", panels.nunique())
            else:
                st.metric("Judge Panels", "N/A")
        with col4:
            if "Purpose of Hearing" in df.columns:
                purposes = df["Purpose of Hearing"].dropna()
                oral = purposes[purposes == "Oral Argument"]
                st.metric("Oral Arguments", len(oral))
            else:
                st.metric("Oral Arguments", "N/A")

        st.divider()
        st.subheader("🔍 Filters")
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            c10_date_filter = st.multiselect(
                "Filter by Date",
                options=safe_sorted_unique(df["Date"]) if "Date" in df.columns else [],
                default=None,
                key="c10_date_filter",
            )
        with col2:
            if "Judges / Panel" in df.columns:
                all_judges = [j for j in safe_sorted_unique(df["Judges / Panel"]) if j]
                c10_judge_filter = st.multiselect(
                    "Filter by Judges",
                    options=all_judges,
                    default=None,
                    key="c10_judge_filter",
                )
            else:
                c10_judge_filter = None
        with col3:
            if "Purpose of Hearing" in df.columns:
                all_purposes = [p for p in safe_sorted_unique(df["Purpose of Hearing"]) if p]
                c10_purpose_filter = st.multiselect(
                    "Filter by Purpose",
                    options=all_purposes,
                    default=None,
                    key="c10_purpose_filter",
                )
            else:
                c10_purpose_filter = None
        with col4:
            c10_search = st.text_input("Search", "", key="c10_search")

        # Apply filters
        filtered = df.copy()
        if c10_date_filter and "Date" in filtered.columns:
            filtered = filtered[filtered["Date"].isin(c10_date_filter)]
        if c10_judge_filter and "Judges / Panel" in filtered.columns:
            filtered = filtered[filtered["Judges / Panel"].isin(c10_judge_filter)]
        if c10_purpose_filter and "Purpose of Hearing" in filtered.columns:
            filtered = filtered[filtered["Purpose of Hearing"].isin(c10_purpose_filter)]
        if c10_search:
            mask = filtered.apply(
                lambda r: c10_search.lower() in str(r).lower(), axis=1
            )
            filtered = filtered[mask]

        st.subheader(f"📋 Cases ({len(filtered)} shown)")

        # Standard 11-field display columns
        display_cols = [
            "Date", "Case Number", "Case Name", "Nature of Case",
            "Court Name", "Location", "Judges / Panel", "Courtroom",
            "Purpose of Hearing", "Time", "Description",
        ]
        available_cols = [c for c in display_cols if c in filtered.columns]
        st.dataframe(filtered[available_cols], use_container_width=True, height=500)

        # --- Charts ---
        st.divider()
        col1, col2, col3 = st.columns(3)
        with col1:
            st.subheader("📊 Cases by Date")
            if "Date" in filtered.columns:
                date_counts = filtered["Date"].value_counts().sort_index()
                date_counts = date_counts[date_counts.index != ""]
                if not date_counts.empty:
                    st.bar_chart(date_counts)
        with col2:
            st.subheader("📊 Hearing Purpose")
            if "Purpose of Hearing" in filtered.columns:
                purpose_counts = filtered["Purpose of Hearing"].value_counts()
                purpose_counts = purpose_counts[purpose_counts.index != ""]
                if not purpose_counts.empty:
                    st.bar_chart(purpose_counts)
        with col3:
            st.subheader("📊 Cases by Courtroom")
            if "Courtroom" in filtered.columns:
                cr_counts = filtered["Courtroom"].value_counts()
                cr_counts = cr_counts[cr_counts.index != ""]
                if not cr_counts.empty:
                    st.bar_chart(cr_counts.head(10))

        # --- Downloads ---
        st.divider()
        col1, col2 = st.columns(2)
        with col1:
            csv = filtered.to_csv(index=False)
            st.download_button(
                "📥 Download Tenth Circuit as CSV",
                data=csv,
                file_name=f"tenth_circuit_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv",
                key="c10_download",
            )
        with col2:
            raw_json = filtered.to_dict(orient="records")
            st.download_button(
                "📥 Download Full JSON",
                data=json.dumps(raw_json, indent=2, ensure_ascii=False),
                file_name=f"tenth_circuit_raw_{datetime.now().strftime('%Y%m%d')}.json",
                mime="application/json",
                key="c10_json_download",
            )

        # --- Raw PDF data expander ---
        with st.expander("📄 View Raw PDF Data"):
            if st.session_state.get("c10_raw_data"):
                for idx, data in enumerate(st.session_state.c10_raw_data, 1):
                    event_title = data.get("event", {}).get("title", "N/A")
                    pdf_url = data.get("pdf_url", "")
                    st.text_area(
                        f"PDF {idx} — {event_title}",
                        value=data.get("text", "")[:5000],
                        height=300,
                        disabled=True,
                        key=f"c10_raw_{idx}",
                    )
                    if pdf_url:
                        st.caption(f"Source: {pdf_url}")
            else:
                st.info("No raw PDF data available.")

        with st.expander("🔧 View Parsed Data (JSON)"):
            raw_json = filtered.to_dict(orient="records")
            st.json(raw_json[:5])
            if len(raw_json) > 5:
                st.caption(f"Showing first 5 of {len(raw_json)} records.")
    else:
        st.info("👈 Click 'Fetch Tenth Circuit' in the sidebar to load data")